...
class AdminController extends Controller
{
    ...
    public function getCompanyReceives(Request $request)
    {
        if($request->has('start_date') && $request->has('end_date')){
            ...
            $ledgers = Ledger::with(['personable'])->whereBetween('created_at', [$startDate, $endDate])->where(['person_type' => 'App\Models\User','person_id'=>1])->where('entry_type','cr')->get();
            return response()->json(['start_date'=>$startDate,'end_date'=>$endDate,'data' => $ledgers]);
        }else{
            $ledgers = Ledger::with(['personable'])->where(['person_type' => 'App\Models\User','person_id'=>1])->where('entry_type','cr')->get();
            return response()->json(['data' => $ledgers]);
        }
    }
